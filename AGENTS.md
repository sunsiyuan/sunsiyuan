...
- 已支持的语法（2026-07-06 逐步补全，别再假设"不支持"）：行内`**粗体**`、`` `行内代码` ``、`[文字](链接)`；块级 ` ```代码块``` `、有序/无序列表（`1. ` / `1 ` / `- `）、段内单个换行会保留成`<br>`（软换行）。**仍不支持**：斜体`*x*`（单星会原样显示星号——文末披露 `*披露：...*` 是特例分支）、图片`![]()`。这仍是手写正则解析器，加新语法前先确认`renderInline()` / `splitBlocks()` / block 循环里是否已支持，别假设标准 markdown 自动生效（2026-07-06 踩过`[](url)`链接、` ``` `代码块、列表都原样显示的坑）。
